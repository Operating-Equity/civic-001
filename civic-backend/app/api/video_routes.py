...
def process_video():
    ...
    try:
        ...
        if request.is_json and 'video_url' in request.json:
            return process_youtube_video(request.json.get('video_url'))
        # Check for file upload in request
        elif request.files and 'video_file' in request.files:
            return process_uploaded_video(request.files['video_file'])
        else:
...
def process_uploaded_video(video_file):
    ...
    try:
        ...
        logger.info(f"Processing uploaded file: {video_file.filename}")
        ...
        try:
            ...
            logger.info(f"Extracting transcript from file: {filepath}")
