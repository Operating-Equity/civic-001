...
| Rate limits | Handled inside the gate; never a failure, never an error on a row. A refusal that arrives anyway (two CIVICs on one key; a cost larger than any seen) sets the bucket to OpenAI's figures; the refused request waits exactly what OpenAI asked and goes first. A used-up quota is reported in words. | OpenAI's own numbers |
| Retries | 8 on a 5xx, which costs nothing; at most 2 on a connection that failed or was cut, which has already spent its tokens. No back-off of ours: a failed attempt rejoins the line at the gate, and OpenAI's own retry-after, when given, comes first. | Never on a model or parameter error |
