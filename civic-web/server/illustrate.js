...
export async function runIllustration({ apiKey, text, signal }) {
  ...
  if (!item) return null;
  if (item.b64_json) return { dataUrl: `data:image/${item.output_format || 'jpeg'};base64,${item.b64_json}`, model: image.model || null };
  if (item.url) return { url: item.url, model: image.model || null };
  return null;
}
